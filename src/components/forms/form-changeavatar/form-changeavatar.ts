    ...
    public constructor(props: FormChangeAvatarProps) {
        ...
        this.events = {
            submit: async (e: Event) => {
                e.preventDefault();
                const submitMessage: HTMLElement = document.querySelector(".submit-message") as HTMLElement;
                ...
                .then(response => {
                    if(response.status == 200) {
                        ...
                        submitMessage.textContent = "Ой! Что-то не так! Сервер пишет " + `${response.status} ${response.response}`;
